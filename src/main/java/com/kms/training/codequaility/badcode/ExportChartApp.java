...
public class ExportChartApp {
	...
	public static void main(String[] args) throws IOException {
		...
		/*
		 * for(String s : args) { if (s.toLowerCase() == "-h") {
		 * System.out.println("usage: ExpChartMain [-h] [-i <arg>] [-o <arg>]");
		 * System.out.println(
		 * "This project is a  training Code Quality (java) meterial");
		 * System.out.println(
		 * " -h             This project is a  training Code Quality (java) meterial."
		 * ); System.out.println(" -t=[bar|pie]   Generate report type");
		 * System.out.println(
		 * " -i=<arg>       Input data file file path (car.txt)");
		 * System.out.println(" -o=<arg>       Output generate report file path"
		 * ); } else if (s.toLowerCase().indexOf("-t") == 0 ) { type =
		 * s.substring( s.indexOf("=") + 1 ); } else if
		 * (s.toLowerCase().indexOf("-i") == 0 ) { inputFile = s.substring(
		 * s.indexOf("=") + 1 ); } else if (s.toLowerCase().indexOf("-o") == 0 )
